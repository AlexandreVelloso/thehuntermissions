...
import WeaponAmmoModel from '../models/WeaponAmmoModel';
...
async function getWeaponsByName(weapons: string[]) {
    return WeaponModel.query()
        .where('name', 'in', weapons);
}

async function insertData(knex: Knex, ammoName: string, weaponsArray: string[]) {
    ...
    if (process.env.NODE_ENV === 'production') {
        ...
        await WeaponAmmoModel.query()
            .insertGraph(weaponsAmmos);
    }
}

async function cableBackedBowArrows(knex: Knex) {
...
async function compoundArrowsRedTracerFlare(knex: Knex) {
    await insertData(knex,
...
async function compoundArrowsStandardBroadhead(knex: Knex) {
    await insertData(knex,
...
async function crossbowArrows(knex: Knex) {
    await insertData(knex,
        'Crossbow Arrows',
        [
...
async function crossbowArrowsBlueTracerFlare(knex: Knex) {
    await insertData(knex,
...
async function crossbowBolts(knex: Knex) {
    await insertData(knex,
        'Crossbow Bolts',
        [
            'Crossbow Pistol',
...
async function cupidArrowsRedTracerFlare(knex: Knex) {
    await insertData(knex,
...
async function dot10GABirdshotShells(knex: Knex) {
    await insertData(knex,
...
async function dot10GABuckshotShells(knex: Knex) {
    await insertData(knex,
...
async function dot10GASlugShells(knex: Knex) {
    await insertData(knex,
        '10 GA Slug Shells',
        [
            '10 GA Lever Action Shotgun',
...
async function dot12GAShellsBirdshot(knex: Knex) {
    await insertData(knex,
...
async function dot12GAShellsBuckshot(knex: Knex) {
    await insertData(knex,
...
async function dot12GAShellsSlug(knex: Knex) {
    await insertData(knex,
        '12 GA Shells (Slug)',
        [
            '12 GA Pump Action Shotgun',
...
async function dot16GABirdshotShells(knex: Knex) {
    await insertData(knex,
...
async function dot16GABuckshotShells(knex: Knex) {
    await insertData(knex,
...
async function dot16GASlugShells(knex: Knex) {
    insertData(knex,
        '16 GA Slug Shells',
        [
            '16GA/9.3x74R Drilling',
...
async function dot17HMR(knex: Knex) {
    await insertData(knex,
        '.17 HMR Ammunition',
...
async function dot20GABirdshotShells(knex: Knex) {
    await insertData(knex,
...
async function dot223NoslerBallisticTip(knex: Knex) {
    await insertData(knex,
...
async function dot22AirRiflePellet(knex: Knex) {
    await insertData(knex,
        '.22 Air Rifle Pellet',
        ['22 Air Rifle']);
}

async function dot22LR(knex: Knex) {
    await insertData(knex,
        '.22 LR Ammunition',
        [
...
async function dot243NoslerBallisticTip(knex: Knex) {
    await insertData(knex,
        '.243 Nosler Ballistic Tip',
        ['243 Bolt Action Rifle']);
}

async function dot300NoslerAccuBond(knex: Knex) {
    await insertData(knex,
        '.300 Nosler AccuBond',
        ['.300 Bolt Action Rifle']);
}

async function dot303BritishAmmunition(knex: Knex) {
    await insertData(knex,
...
async function dot308NoslerPartition(knex: Knex) {
    await insertData(knex,
...
async function dot30Minus06NoslerPartition(knex: Knex) {
    await insertData(knex,
...
async function dot30Minus06RoundNoseAmmunition(knex: Knex) {
    await insertData(knex,
...
async function dot30Minus30NoslerPartition(knex: Knex) {
    await insertData(knex,
...
async function dot30RAmmunition(knex: Knex) {
    await insertData(knex,
        '.30 R Ammunition',
        [
...
async function dot340WeatherbyMagnumAmmunition(knex: Knex) {
    await insertData(knex,
        '.340 Weatherby Magnum Ammunition',
        ['340 Weatherby Magnum Bolt Action Rifle']);
}

async function dot357NoslerSportingHandgun(knex: Knex) {
    await insertData(knex,
...
async function dot405Ammunition(knex: Knex) {
    await insertData(knex,
        '.405 Ammunition',
        [
...
async function dot44NoslerSportingHandgun(knex: Knex) {
    await insertData(knex,
...
async function dot454RevolverHandgun(knex: Knex) {
    await insertData(knex,
...
async function dot45CopperBullet(knex: Knex) {
    await insertData(knex,
        '.45 Copper Bullet',
        [
...
async function dot45LongColtHandgun(knex: Knex) {
    await insertData(knex,
...
async function dot45Minus70Government(knex: Knex) {
    await insertData(knex,
        '.45-70 Government',
        [
...
async function dot45SabotedBullet(knex: Knex) {
    await insertData(knex,
        '.45 Saboted Bullet',
        [
...
async function dot50LeadBullet(knex: Knex) {
    await insertData(knex,
        '.50 Lead Bullet',
        [
            '50 Inline Muzzleloading Pistol',
...
async function dot50PatchedRoundBall(knex: Knex) {
    await insertData(knex,
...
async function dot6dot5x55NoslerAccuBond(knex: Knex) {
    await insertData(knex,
...
async function dot7dot62x54RSPAmmunition(knex: Knex) {
    await insertData(knex,
...
async function dot7mmMagnum(knex: Knex) {
    await insertData(knex,
        '7mm Magnum',
        [
...
async function dot7mmMinus08AmmunitionHP(knex: Knex) {
    await insertData(knex,
...
async function dot7mmMinus08AmmunitionHV(knex: Knex) {
    await insertData(knex,
...
async function dot7x64mmAmmunition(knex: Knex) {
    await insertData(knex,
        '7x64mm Ammunition',
        [
...
async function dot8x57ISNoslerPartition(knex: Knex) {
    await insertData(knex,
...
async function dot9dot3x62NoslerPartition(knex: Knex) {
    await insertData(knex,
...
async function dot9dot3x74RAmmunition(knex: Knex) {
    await insertData(knex,
        '.9.3x74R Ammunition',
        [
...
async function longbowArrows(knex: Knex) {
    await insertData(knex,
        'Longbow Arrows',
        [
            'Longbow',
...
async function recurveArrowsStandardBroadhead(knex: Knex) {
    await insertData(knex,
...
async function recurveArrowsOrangeTracerFlare(knex: Knex) {
    await insertData(knex,
...
async function silver308Bullets(knex: Knex) {
    await insertData(knex,
        'Silver .308 Bullets',
        ['.308 "Wolfsbane" Handgun']);
}
...
async function silverCrossbowBolts(knex: Knex) {
    await insertData(knex,
